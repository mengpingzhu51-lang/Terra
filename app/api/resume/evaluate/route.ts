import { evaluationSchema } from "@/lib/ai";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const parsed = await generateStructuredObject({
      systemPrompt: "你是一个最顶级的HRD与大厂面试官，拥有审视候选人经历契合度的犀利眼光。请从客观事实出发，找出亮点与雷区，并依据STAR原则（背景、任务、动作、成果）给出精准的、令人信服的、切中JD要求的各项匹配评分与针对性改写指南。注意：分条清晰、内容饱满，且结果必须完全符合JSON结构定义。",
      userPrompt: prompt,
      schema: evaluationSchema,
      threadId: `resume-evaluate-${Date.now()}`,
    });
